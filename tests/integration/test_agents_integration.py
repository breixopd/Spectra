...
async def test_scope_agent_flow(mock_llm):
    ...
    mock_llm.structured_responses = {
        "ScopeAction": {
            ...
            "targets": [
                {"value": "1.1.1.1", "target_type": "ip", "resolved_ips": [], "ports": [], "notes": ""}
            ],
            "exclusions": [],
            "total_hosts": 1,
            "warnings": []
...
async def test_exploit_crafter_flow(mock_llm):
    ...
    from app.services.ai.agents.exploit_crafter import ExploitCrafter, ExploitInput, ExploitAction
...
async def test_mission_controller_planning(mock_llm):
    ...
    from app.services.ai.agents.mission_controller import MissionController, MissionInput, MissionPlan, MissionType
    ...
    mock_llm.structured_responses = {
        "MissionPlan": {
            ...
            "tasks": [
                {"task_id": "t1", "description": "Scan", "agent_type": "tool_selector", "phase": "discovery", "priority": 1}
            ],
            "current_phase": "scope",
            "confidence": 0.9,
            "risk_level": "low",
    ...
    with patch("app.services.ai.knowledge.get_available_tools_context", new_callable=MagicMock) as mock_tools, \
