...
class TestRecordOutcome:
    def test_records_single_outcome(self):
        ...
        outcome = ScanOutcome(tool_name="nmap", target_type="web", template_or_args="-sV", findings_count=3)
    ...
    def test_records_multiple_outcomes(self):
        scanner = AdaptiveScanner()
        for i in range(5):
            scanner.record_outcome(
                ScanOutcome(
                    tool_name="nuclei",
                    target_type="web",
                    template_or_args="cves",
                    findings_count=i,
                )
            )
...
class TestShouldSkip:
    def _populate(self, scanner: AdaptiveScanner, tool: str, target: str, template: str, count: int, findings: int = 0):
        for _ in range(count):
            scanner.record_outcome(
                ScanOutcome(
                    tool_name=tool,
                    target_type=target,
                    template_or_args=template,
