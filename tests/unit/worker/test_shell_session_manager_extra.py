...
from spectra_infra.shell.session_manager import ShellSession, ShellSessionManager
...
class TestShellSessionManagerStartListener:
    def test_start_with_specific_port(self, manager):
        with (
            patch("spectra_infra.shell.session_manager._get_service_mode", return_value="worker"),
    ...
    def test_start_on_already_active_port(self, manager):
        manager.listeners[4444] = MagicMock()
        with patch("spectra_infra.shell.session_manager._get_service_mode", return_value="worker"):
            port = manager.start_listener("s1", "10.0.0.1", port=4444)
        assert port == 4444

    def test_direct_listener_denied_outside_worker_mode(self, manager):
        with patch("spectra_infra.shell.session_manager._get_service_mode", return_value="api"):
