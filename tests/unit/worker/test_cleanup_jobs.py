...
async def test_cleanup_expired_cache_purges_entries():
    from spectra_system.maintenance_cleanup import cleanup_expired_cache
...
async def test_cleanup_completed_jobs_removes_old():
    from spectra_system.maintenance_cleanup import cleanup_completed_jobs
...
async def test_run_all_cleanup_calls_all_functions():
    ...
    with (
        patch("spectra_persistence.database.async_session_maker", return_value=ctx),
        patch("spectra_tools.sandbox.get_sandbox_pool", return_value=mock_pool),
