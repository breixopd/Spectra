...
async def test_verify_system_requirements_all_pass(mock_setup_service):
    ...
    with patch.object(mock_setup_service, "check_database", new_callable=AsyncMock) as mock_db, \
         patch.object(mock_setup_service, "check_redis", new_callable=AsyncMock) as mock_redis, \
...
async def test_verify_system_requirements_failure(mock_setup_service):
    ...
    with patch.object(mock_setup_service, "check_database", new_callable=AsyncMock) as mock_db, \
        ...
        with patch.object(mock_setup_service, "check_docker", new_callable=AsyncMock) as m_d, \
