...
from app.services.tools.models import RegisteredTool, ToolConfig, ToolStatus, ToolCategory, ToolCapability
...
async def test_stealth_mode_enforcement(tool_selector, mock_nmap_tool):
    ...
    assert "-T4" not in overridden_args # Should strip aggressive flags keys if they were keys (bad dict usage but consistent with logic)
