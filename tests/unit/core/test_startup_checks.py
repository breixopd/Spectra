"""
Tests for startup checks in spectra_api.bootstrap.lifespan.run_startup_checks().
...
async def test_db_connectivity_failure(caplog):
    ...
    with patch("spectra_api.bootstrap.lifespan.async_session_maker", side_effect=ConnectionError("refused")):
...
async def test_disk_space_ok(caplog, tmp_path):
    ...
    with patch("spectra_api.bootstrap.lifespan.async_session_maker", side_effect=session_factory):
        with patch("spectra_api.bootstrap.lifespan.data_root", return_value=tmp_path):
            with patch("spectra_api.bootstrap.lifespan.shutil.disk_usage", return_value=fake_usage):
