...
class TestExploitCrafter:
    ...
    async def test_execute_with_candidates_first_attempt(self, mock_llm, context):
        ...
        with (
            patch.object(agent, "_find_exploit_candidates", new_callable=AsyncMock, return_value=candidates),
            patch.object(agent, "_configure_exploit", new_callable=AsyncMock, return_value=mock_action),
        ):
            input_data = ExploitInput(target="10.0.0.1")
            result = await agent.execute(context, input_data)
    ...
    async def test_execute_rotates_candidates(self, mock_llm, context):
        ...
        with (
            patch.object(agent, "_find_exploit_candidates", new_callable=AsyncMock, return_value=candidates),
            patch.object(agent, "_configure_exploit", new_callable=AsyncMock, return_value=mock_action) as mock_cfg,
        ):
            input_data = ExploitInput(target="10.0.0.1")
            result = await agent.execute(context, input_data)
...
class TestVectorGeneratorAgent:
    ...
    async def test_execute_generates_vectors(self, mock_llm, context):
        ...
        with patch.object(agent, "_generate_with_llm", new_callable=AsyncMock, return_value=mock_output):
    ...
    async def test_execute_handles_llm_failure(self, mock_llm, context):
        ...
        with patch.object(
            agent, "_generate_with_llm", new_callable=AsyncMock, side_effect=RuntimeError("LLM error")
        ):
            input_data = VectorGeneratorInput(
                target_type="service",
