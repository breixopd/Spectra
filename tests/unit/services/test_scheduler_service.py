...
async def test_sandbox_watchdog_loop_runs_single_iteration():
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setitem(
            sys.modules,
            "spectra_infra.background_tasks",
            make_module("spectra_infra.background_tasks", sandbox_watchdog_loop=watchdog),
        )
...
async def test_quota_reset_runs_single_iteration():
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setitem(
            sys.modules,
            "spectra_billing.usage_tracker",
            make_module("spectra_billing.usage_tracker", UsageTracker=lambda: tracker),
        )
...
async def test_metrics_collector_runs_single_iteration():
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setitem(
            sys.modules,
            "spectra_infra.metrics_store",
            make_module("spectra_infra.metrics_store", get_metrics_store=lambda: store),
        )
        mp.setattr(scheduler_locking, "advisory_lock_owner", fake_lock_owner)
        mp.setattr(scheduler_async_ops, "sleep", AsyncMock())
