...
def _make_s3_storage():
    ...
    with (
        patch("spectra_common.config.settings") as mock_settings,
        patch("spectra_storage_policy.storage.service._import_s3_deps") as mock_deps,
    ):
...
class TestStorageServiceInit:
    ...
    def test_missing_endpoint_raises_runtime_error(self):
        ...
        with patch("spectra_common.config.settings") as s:
            s.S3_ENDPOINT_URL = ""
...
class TestStorageServiceS3Mode:
    ...
    async def test_health_check_s3_healthy(self, svc):
        ...
        with patch("spectra_common.config.settings") as ms:
...
class TestStorageServiceSingleton:
    ...
    async def test_close_storage_service(self):
        import spectra_storage_policy.storage.service as svc_mod
        from spectra_storage_policy.storage.service import close_storage_service
