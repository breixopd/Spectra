...
def mock_manager_context():
    ...
    with (
        patch("spectra_mission.manager.execution.MissionExecutor") as MockExecutor,
        patch("spectra_mission.manager.execution.MissionController") as MockController,
        patch("spectra_mission.manager.execution.ScopeAgent") as MockScope,
        ...
        patch("spectra_mission.manager.execution.VotingSystem") as MockVoting,
        patch("spectra_mission.manager.lifecycle.MissionRepository") as MockRepo,
        patch(
            "spectra_mission.manager.execution.get_global_llm_client",
            new_callable=AsyncMock,
        ) as mock_get_llm,
        patch("spectra_persistence.database.async_session_maker") as mock_session_maker,
        patch("spectra_mission.manager.lifecycle.async_session_maker") as mock_lifecycle_session,
        patch("spectra_mission.manager.lifecycle.resolve_ip", new_callable=AsyncMock) as mock_resolve_ip,
        patch("spectra_mission.state_store.async_session_maker") as mock_state_store_session,
        patch("spectra_billing.quota_enforcer.async_session_maker") as mock_quota_session,
    ):
...
async def test_start_mission(mock_manager_context):
    ...
    with patch("spectra_mission.manager.asyncio.create_task", side_effect=_safe_create_task) as mock_create_task:
...
async def test_run_mission_loop_success(mock_manager_context):
    ...
    from spectra_ai_core.agents.mission_controller import MissionType
...
async def test_adaptive_replanning(mock_manager_context):
    ...
    from spectra_ai_core.agents.base import ActionRisk
...
async def test_requirements_injection_sanitized(mock_manager_context):
    ...
    with patch("spectra_mission.manager.asyncio.create_task", side_effect=_safe_create_task):
