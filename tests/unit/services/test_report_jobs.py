...
async def test_generate_mission_report_returns_path():
    ...
    with (
        patch("spectra_persistence.database.async_session_maker", return_value=_mock_session_ctx(session)),
        patch("spectra_ai_core.llm.get_global_llm_client", new_callable=AsyncMock, return_value=MagicMock()),
        patch("spectra_ai_core.agents.reporter.ReporterAgent", return_value=mock_reporter),
    ):
