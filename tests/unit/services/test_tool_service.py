...
from spectra_ai_core.agents.base import ToolAction
from spectra_tools.output import create_error_result, normalize_tool_name, validate_tool_name
...
def tool_service(mock_llm):
    with patch("spectra_tools.service.SafetySupervisorAgent"), patch("spectra_tools.service.VotingSystem"):
...
class TestToolServiceExecution:
    ...
    async def test_execute_tool_not_found(self, tool_service):
        ...
        with patch("spectra_tools.validation.get_registry") as mock_reg:
    ...
    async def test_execute_custom_script(self, tool_service):
        ...
        with patch("spectra_infra.queue.PostgresJobQueue") as MockQueue, patch("spectra_infra.queue.Job") as MockJob:
    ...
    async def test_execute_custom_script_exception(self, tool_service):
        ...
        with patch("spectra_infra.queue.PostgresJobQueue", side_effect=RuntimeError("queue fail")):
            result = await tool_service.execute_custom_script(
                mission=mission,
                script_content="code",
                language="python",
