...
from spectra_mission.chain_builder import (
    BUILTIN_CHAINS,
    ChainBuilder,
...
class TestCustomChains:
    def test_load_nonexistent_returns_empty(self):
        with patch("spectra_mission.chain_builder.CUSTOM_CHAINS_PATH", Path("/nonexistent")):
    ...
    def test_load_invalid_json_returns_empty(self, tmp_path):
        ...
        with patch("spectra_mission.chain_builder.CUSTOM_CHAINS_PATH", bad_file):
            result = load_custom_chains()
            assert result == []
    ...
    def test_save_and_load_custom_chain(self, tmp_path):
        ...
        with patch("spectra_mission.chain_builder.CUSTOM_CHAINS_PATH", chains_file):
            with patch("spectra_mission.chain_builder.load_custom_chains", return_value=[]):
