...
from spectra_tools.sandbox.image_scanner import ImageScanner, ScanResult
...
async def test_get_last_scan_exception():
    ...
    with patch("spectra_persistence.database.async_session_maker", side_effect=RuntimeError("db error")):
        result = await scanner.get_last_scan()
