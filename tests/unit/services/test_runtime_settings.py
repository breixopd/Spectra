...
def test_apply_general_runtime_settings_sets_expected_fields():
    ...
    with patch("spectra_system.runtime_settings.settings") as mock_settings:
...
async def test_hydrate_runtime_settings_applies_rows_and_resets_caches(monkeypatch):
    ...
    with (
        patch("spectra_system.runtime_settings.settings") as mock_settings,
        patch(
            "spectra_system.runtime_settings.reset_runtime_ai_caches",
...
async def test_lifespan_hydrates_runtime_before_embedding_init():
    ...
    with contextlib.ExitStack() as stack:
        ...
        mock_init_registry = stack.enter_context(
            patch("spectra_tools_core.registry.initialize_registry", new_callable=AsyncMock)
        )
        ...
        stack.enter_context(patch("spectra_storage_policy.storage.close_storage_service", new_callable=AsyncMock))
        stack.enter_context(patch("spectra_ai_core.embeddings.EmbeddingService", FakeEmbeddingService))
        stack.enter_context(patch("spectra_mission.core.bridge.EventWebSocketBridge", return_value=FakeBridge()))
        ...
        stack.enter_context(
            patch(
                "spectra_storage_policy.storage.get_storage_service",
...
async def test_hydrate_bootstrap_only_vars_skip_env_override():
    ...
    with (
        patch("spectra_system.runtime_settings.settings"),
        patch(
            "spectra_system.runtime_settings.reset_runtime_ai_caches",
            new_callable=AsyncMock,
        ),
