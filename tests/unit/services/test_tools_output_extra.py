...
from spectra_tools.output import persist_output_directory, record_to_memory
...
async def test_persist_output_directory(tmp_path):
    ...
    with patch("spectra_storage_policy.storage.get_storage_service", return_value=mock_storage):
...
async def test_record_to_memory():
    ...
    with patch("spectra_ai_core.memory.get_memory", return_value=mock_memory):
        with patch("spectra_ai_core.memory.detect_os_from_output", return_value="linux"):
...
async def test_record_to_memory_no_os():
    ...
    with patch("spectra_ai_core.memory.get_memory", return_value=mock_memory):
        with patch("spectra_ai_core.memory.detect_os_from_output", return_value="unknown"):
