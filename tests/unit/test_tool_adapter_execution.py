...
from app.services.tools.models import ToolConfig, ToolExecutionRequest, ToolExecutionResult
...
async def test_execute_docker_command(adapter):
    ...
    with patch("app.services.tools.adapter.runner.settings.TOOL_CONTAINER_NAME", "spectra-tools"), \
         patch("asyncio.create_subprocess_shell") as mock_subprocess:
