...
from app.services.ai.agents.base import AgentContext, AgentResult, ToolAction, ActionRisk
...
async def test_dispatch_delegates_correctly(dispatcher, mock_mission, mock_context):
    ...
    task = Task(task_id="t1", description="test", agent_type="tool_selector", phase=AssessmentPhase.DISCOVERY)
...
async def test_dispatch_unknown_agent_fallback(dispatcher, mock_mission, mock_context):
    ...
    task = Task(task_id="t1", description="test", agent_type="discovery", phase=AssessmentPhase.DISCOVERY)
...
async def test_handle_payload_crafter_flow(dispatcher, mock_mission, mock_context):
    ...
    task = Task(task_id="t1", description="Exploit", agent_type="payload_crafter", phase=AssessmentPhase.EXPLOITATION)
...
async def test_update_attack_surface_service(dispatcher, mock_mission):
    ...
    finding = {"host": "1.2.3.4", "port": 80, "service": "http", "product": "nginx", "version": "1.0"}
...
async def test_scope_handler(dispatcher, mock_mission, mock_context):
    ...
    task = Task(task_id="t1", description="Scope", agent_type="scope", phase=AssessmentPhase.SCOPE)
...
async def test_reporter_handler(dispatcher, mock_mission, mock_context):
    ...
    task = Task(task_id="t1", description="Report", agent_type="reporter", phase=AssessmentPhase.REPORTING)
