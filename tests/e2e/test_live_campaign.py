...
class TestLiveCampaign:
    ...
    async def run_mission_until_completion(self, mission_manager, mission_id, timeout=1800):
        ...
        while elapsed < max_wait:
            ...
            if mission.status in ["completed", "failed", "stopped", "cancelled", "exploitation_successful", "exploitation_exhausted"]:
    ...
    async def test_campaign_server_exploitation(self, real_mission_manager):
        ...
        assert mission.status in ["completed", "exploitation_successful", "exploitation_exhausted"], f"Mission failed with status: {mission.status}"
    ...
    def _print_mission_summary(self, mission):
        ...
        print(f"Attack Surface: {len(mission.attack_surface.services)} services, {len(mission.attack_surface.vulnerabilities)} vulns")
