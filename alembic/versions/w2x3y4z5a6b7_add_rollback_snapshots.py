...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "rolled_back", sa.Boolean(), nullable=False, server_default="false"
        ),
...
def downgrade() -> None:
    op.drop_index(
        "ix_rollback_snapshots_target_entity_id", table_name="rollback_snapshots"
    )
    op.drop_index(
        "ix_rollback_snapshots_actor_user_id", table_name="rollback_snapshots"
    )
