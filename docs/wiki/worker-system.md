...
[← Wiki Home](home.md) | [Architecture](architecture.md) | [Configuration](configuration.md)
...
Use `scripts/ops/worker_management.sh` when you need to inspect or repair the worker queue outside the UI.

- `status`, `pending`, `failed`, and `dead-letter` provide quick queue visibility
- `retry-failed` and `retry-job <id>` move work back to `pending`
- `purge-completed` and `purge-dead` clean up old queue records
- `worker-health` checks the worker container and running processes
