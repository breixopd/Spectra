...
- `SERVICE_MODE` env var on each image for shared settings (e.g. DB pool hints in `app.core.config`) and lifespan branches; **not** six-way router modes on one process
