...
class AdaptiveScanner:
    ...
    def record_outcome(self, outcome: ScanOutcome):
        ...
        logger.debug(
            "Recorded outcome: %s on %s -> %d findings", outcome.tool_name, outcome.target_type, outcome.findings_count
        )

    def should_skip(self, tool_name: str, target_type: str, template_or_args: str | None = None) -> tuple[bool, str]:
        """Determine if a scan should be skipped based on historical data.

        Returns (should_skip, reason).
        """
        matching = [o for o in self._outcomes if o.tool_name == tool_name and o.target_type == target_type]
        ...
        if template_or_args:
            ...
            if len(template_matching) >= self._min_samples:
                ...
                if yield_rate < self._skip_threshold:
                    return (
                        True,
                        f"Template '{template_or_args}' has {yield_rate:.0%} yield rate on {target_type} targets ({len(template_matching)} samples)",
                    )
        ...
        if len(matching) >= self._min_samples:
            ...
            if yield_rate < self._skip_threshold:
                return (
                    True,
                    f"{tool_name} has {yield_rate:.0%} yield rate on {target_type} targets ({len(matching)} samples)",
                )
    ...
    def get_recommended_tools(self, target_type: str, limit: int = 5) -> list[dict]:
        ...
        for o in self._outcomes:
            if o.target_type == target_type:
                ...
                stats["avg_duration"] = (
                    stats["avg_duration"] * (stats["total"] - 1) + o.scan_duration_seconds
                ) / stats["total"]
        ...
        data = [
            {
                ...
                "template": o.template_or_args,
                "findings": o.findings_count,
                "fp": o.false_positive_count,
                "duration": o.scan_duration_seconds,
    ...
    async def restore(self):
        ...
        try:
            async with async_session_maker() as session:
                result = await session.execute(text("SELECT value FROM system_cache WHERE key = 'adaptive_scan_data'"))
                row = result.fetchone()
                if row:
                    ...
                    for d in data:
                        self._outcomes.append(
                            ScanOutcome(
                                ...
                                template_or_args=d["template"],
                                findings_count=d["findings"],
