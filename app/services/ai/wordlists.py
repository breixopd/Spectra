...
# Performance Optimization: Pre-compute static dictionaries at module level
# to avoid recreating them on every function call
TECH_PATHS = {
...
def generate_credential_list(service: str, product: str | None = None) -> dict[str, list[str]]:
    ...
    # Performance Optimization: Use dictionary comprehension to avoid mutating the global default
    base_creds = SERVICE_CREDS.get(service.lower(), SERVICE_CREDS["http"])
    creds = {k: list(v) for k, v in base_creds.items()}
