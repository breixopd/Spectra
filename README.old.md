...
3. **Open the web UI:**
   ...
   - Configure your AI provider (OpenAI-compatible API endpoint for hosted models, Ollama for local GPU)
...
7. **RAG indexes mission results** for semantic search in future engagements (embedding service is API-only; no SHA256 fallback)
...
By default everything runs in-process on a single server. Two services can be extracted to remote servers when you need to scale:

| Service | Env Var | Protocol | When to Extract |
| ------- | ------- | -------- | --------------- |
| **LLM Inference** | `LLM_GATEWAY_URL` | HTTP (LiteLLM/Ollama proxy) | GPU-heavy workloads, dedicated inference server |
| **Sandbox Orchestrator** | `SANDBOX_ORCHESTRATOR_URL` | HTTP (Docker/K8s API) | Run tool containers on dedicated worker nodes |

**Single-server mode** (default): Leave all gateway URLs empty. The `ServiceRegistry` (`app/services/gateway/service_registry.py`) automatically uses in-process implementations.

**Multi-server mode**: Set one or more gateway URLs via:
- Environment variables in `.env`
- The setup wizard at `/setup` (Service Topology step)
- Admin panel → Services tab (runtime changes)

**SSH auto-provisioning**: From the admin panel, provide SSH credentials for a remote server and Spectra will install Docker, pull images, start the service, and verify health — one click.
...
│   │   ├── embeddings.py # Embedding service (API/SHA256 fallback)
