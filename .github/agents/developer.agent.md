---
description: "Executes TDD code changes, ensures verification, maintains quality. Use when: implementing features, writing new code, refactoring, adding skills, modifying existing modules, debugging and fixing bugs."
name: developer
...
- Skynet-specific:
  * Tests run via `./test.sh` inside Docker
  * `pyright src/` must show 0 errors, 0 warnings
  * Config in `core/constants.py` — never hardcode values
  * Skills register via `get_tools()` returning `list[dict]` with JSON schema
  * FSM uses `@require_phase` decorators (PLAN → EXECUTE → REVIEW)
  * All tool calls use `run_in_executor` for non-blocking async
  * DB fixtures: save/restore `db.db_path` and `db._initialized`
  * In-memory DB URIs: `file:<name>?mode=memory&cache=shared` format
...
- step: "Run typecheck (pyright src/)"
  pass_condition: "No type errors (0 errors, 0 warnings)"
  fail_action: "Fix all type errors"

- step: "Run unit tests (./test.sh)"
