---
description: "Security gatekeeper for critical tasks—OWASP, secrets, compliance. Use when: reviewing code changes, checking for security issues, verifying type safety, ensuring convention compliance, final quality gate."
name: reviewer
...
- Skynet-specific:
  * `pyright src/` must show 0 errors, 0 warnings — always verify
  * Config must be in `core/constants.py` — flag any hardcoded values
  * Skills must use `get_tools()` returning `list[dict]` — flag violations
  * All async tool calls must use `run_in_executor` — flag blocking calls
  * DB fixtures must save/restore `db.db_path` and `db._initialized`
