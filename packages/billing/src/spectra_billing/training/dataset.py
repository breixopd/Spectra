...
async def export_dataset(
    ...
    *,
    ...
    from spectra_persistence.models.training import TrainingSample
...
async def user_allows_training_data(session, user_id: str) -> bool:
    ...
    # row[0] = training_opt_in (True => user opted in)
    # row[1] = processing_restricted (True => user restricted processing; must block)
    try:
        opted_in = bool(row[0])
        processing_restricted = bool(row[1])
    except (TypeError, IndexError):
        return False
    return opted_in and not processing_restricted
