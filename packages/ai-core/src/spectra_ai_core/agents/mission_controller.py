...
from spectra_ai_core.errors import AgentError, LLMParseError, LLMTimeoutError
from spectra_ai_core.prompts import (
    MISSION_PLAN_PROMPT,
)
...
class MissionController(Agent[MissionInput, MissionPlan | PhaseTransition | SteeringAction]):
    ...
    async def execute(
        ...
        try:
            ...
            try:
                from spectra_ai_core.agents.safety import SafetyInput
    ...
    async def _create_mission_plan(
        ...
        lessons_context = ""
        try:
            from spectra_ai_core.memory import get_memory as _get_memory
