...
class ServerPoolManager:
    ...
    async def _auto_enable_autoscale(self, session: AsyncSession) -> None:
        ...
        import spectra_platform.core.config as config

        if config.settings.AUTOSCALE_ENABLED:
            return  # already enabled
        try:
            from spectra_platform.services.system.runtime_settings import upsert_system_config_values
    ...
    async def register_local_node(self) -> dict:
        ...
        async with async_session_maker() as session:
            ...
            from spectra_platform.services.scaling.docker_client import list_nodes
    ...
    async def get_cluster_capacity(self) -> dict:
        ...
        """
        from spectra_platform.models.server_node import ServerNode
        from spectra_platform.services.scaling.config import DEFAULT_RESOURCE_REQUIREMENTS
