...
def _encrypt_config_value(value: str) -> str:
    ...
    if not value or value.startswith(_FERNET_TOKEN_PREFIX):
        return value
    try:
        from spectra_platform.auth.encryption import _get_default_secret, encrypt_field
...
def _decrypt_config_value(value: str) -> str:
    ...
    if not value:
        return value
    try:
        from spectra_platform.auth.encryption import _get_default_secret, decrypt_field
